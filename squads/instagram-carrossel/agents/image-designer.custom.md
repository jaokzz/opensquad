...
- **Identidade visual: 3 casos, nessa ordem de prioridade.**
  ...
  2. **Marca própria do João (@jaokdev), sem cliente específico** (ex: conteúdo sobre o próprio serviço, autoridade, bastidores) → usar o design system fixo em `pessoal/carrosseis/DESIGN-SYSTEM.md` (Barlow Condensed, preto/branco alternado, acento laranja-avermelhado) em vez de inventar ou reusar a paleta genérica de `research-brief.md`. Além do sistema default (v2), existe uma **biblioteca de 5 templates de layout** em `pessoal/carrosseis/templates/README.md` (confronto editorial, bastidores lifestyle, mockup de ferramenta, manchete sobre foto, meme/sticker) — consultar a tabela de decisão de lá quando o conteúdo pedir um formato diferente do padrão autoridade/dado, ou quando o João pedir um modelo específico pelo nome/número. Templates são compatíveis entre si (mesma fundação de cor/fonte/header) e podem ser mesclados no mesmo carrossel.
