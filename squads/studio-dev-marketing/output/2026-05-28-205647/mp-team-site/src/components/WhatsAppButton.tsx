...
export default function WhatsAppButton() {
  ...
  useEffect(() => {
    ...
    const pingTimer = setTimeout(() => setPing(false), 8000)
    return () => { clearTimeout(showTimer); clearTimeout(pingTimer) }
  }, [])
          ...
          <AnimatePresence>
            {tip && (
              <motion.div
                ...
                <p className="text-[13px] font-bold text-white">Falar com a Pietra</p>
                <p className="text-[11px]" style={{ color: '#7A7A8C' }}>Resposta rápida</p>
              </motion.div>
            )}
          </AnimatePresence>

          <motion.a
            href={WA_GERAL} target="_blank" rel="noopener noreferrer"
